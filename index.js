...
function getConsensus(candles) {
  ...
  let bv = 0, bev = 0;
  w.forEach(c => {
    ['wobi', 'obi_roc', 'aggression'].forEach(k => {
      const v = c[k] || 0;
      if (v > VOTE_THRESHOLD) bv++;
      else if (v < -VOTE_THRESHOLD) bev++;
    });
  });
  const vr = bv >= VOTE_PASS ? 'BULL' : bev >= VOTE_PASS ? 'BEAR' : 'NONE';
  if (vr === 'NONE') return 'NONE';
  const cdNow = candles[candles.length - 1].cum_delta;
  const cdThen = candles[candles.length - LOOKBACK].cum_delta;
  const chg = cdNow - cdThen;
  if (vr === 'BULL' && chg > 0) return 'BULL';
  if (vr === 'BEAR' && chg < 0) return 'BEAR';
  return 'BLOCKED';
}

function getOrderBookSnapshot() {
  const bids = Object.entries(orderBook.bids).sort((a, b) => parseFloat(b[0]) - parseFloat(a[0])).slice(0, 20);
  const asks = Object.entries(orderBook.asks).sort((a, b) => parseFloat(a[0]) - parseFloat(b[0])).slice(0, 20);
  return { bids, asks };
}
...
async function sealCandle() {
  ...
  try {
    ...
    obiHistory.push(obi);
    const prevObi = obiHistory.length > LOOKBACK ? obiHistory[obiHistory.length - LOOKBACK - 1] : obi;
    ...
    else console.log(`Candle #${candleCount} | close=${sealed.close} | delta=${sealed.delta.toFixed(4)} | consensus=${sealed.consensus}`);
...
function processTick(price, qty, isBuyerMaker, timestamp) {
  totalTicks++;
  if (totalTicks % 500 === 0) {
    console.log(`Ticks: ${totalTicks} | candles: ${candleCount} | cum_delta: ${cumulativeDelta.toFixed(3)}`);
  }
  ...
  if (!currentCandle) {
    currentCandle = { tsOpen: timestamp, open: price, high: price, low: price, close: price, volume: 0, buyVol: 0, sellVol: 0, tradeCount: 0 };
  }
  ...
  if (currentCandle.volume >= CANDLE_SIZE_BTC) sealCandle();
}

app.get('/candles', async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 500;
    const { data, error } = await supabase.from('candles').select('*').order('candle_num', { ascending: true }).limit(limit);
...
function connectBook() {
  ...
  ws.on('close', () => { setTimeout(connectBook, 3000); });
...
function connectTrades() {
  ...
  ws.on('message', (data) => {
    ...
    } catch (e) { console.error('Trade parse error:', e.message); }
  });
  ws.on('close', () => { console.log('Trade stream closed, reconnecting...'); setTimeout(connectTrades, 3000); });
  ws.on('error', (e) => { console.error('Trade error:', e.message); ws.terminate(); setTimeout(connectTrades, 5000); });
}

connectBook();
connectTrades();
console.log('BTC Order Flow server starting — Southeast Asia region');
